...
if not sympy_err:
    ...
    post_vars_ = ['wallShearStress']
    var_dict_ = {'wallShearStress': 1}
    x_positions_ = [0]
    ...
    try:
        ...
        if (max_iter==0) | (max_iter==20000):
            ...
            for var in post_vars_:
                ...
                for pos in x_positions_:
                    post_file = 'Y' + str(int(pos)) + 'H_' + var + '.csv'
                    data = np.loadtxt(os.path.join(post_folder, str(max_iter), post_file), delimiter=',', skiprows=1)
                    var_data = np.append(var_data, data[:,var_dict_[var]]/(-0.5*0.697643*0.697643))

                save_file = 'CF1.edf.gz'
                #save_file = var.upper() + '1.edf.gz'
                np.savetxt("../output/" + save_file, var_data)
    except:
        sim_err_ = 1
...
if (sympy_err + sim_err) > 0:
    ...
    save_file = 'CF1.edf.gz'
    np.savetxt("./output/" + save_file, [np.nan]*77)
