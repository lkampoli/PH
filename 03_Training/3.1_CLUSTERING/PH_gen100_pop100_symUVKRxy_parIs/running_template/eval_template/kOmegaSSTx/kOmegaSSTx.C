...
:
    ...
    (
        ...
        (
            ...
            runTime_.timeName(),
            mesh_,
            IOobject::NO_READ, //NO_READ, // NO_READ = 0, MUST_READ = 1, MUST_READ_IF_MODIFIED = 3, READ_IF_PRESENT
            IOobject::AUTO_WRITE
        ),
        mesh_,
        dimensionedScalar("cluster",dimensionSet(0,0,0,0,0,0,0), 9999)
    )
...
void kOmegaSSTx::correct()
{
    ...
    Info << "Reading labels.dat file ..." << endl;
    ifstream file;
    ...
    forAll(Ax_,id)
    {
          //Info << ClusterId[id] << endl;
          cluster_[id] = ClusterId[id];
    }
    Info << " ClusterId labelled " << endl;

    // Update models for each cluster
    forAll(Ax_, idx)
    {
	    if (ClusterId[idx] == 0) // cluster 0
	    {
            //Info << " Cluster 0 " << endl;
	        Ax_[idx] = Ax0_[idx];
	        Rx_[idx] = Rx0_[idx];
	    }
	    else if (ClusterId[idx] == 1) // cluster 1
	    {
            //Info << " Cluster 1 " << endl;
	        Ax_[idx] = Ax1_[idx];
	        Rx_[idx] = Rx1_[idx];
	    }
	    else if (ClusterId[idx] == 2) // cluster 2
	    {
            //Info << " Cluster 2 " << endl;
	        Ax_[idx] = Ax2_[idx];
	        Rx_[idx] = Rx2_[idx];
